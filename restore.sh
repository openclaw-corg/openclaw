...
# Restore runtime config (~/.openclaw/)
if [ -f "$BACKUP_DIR/openclaw-runtime.tar.gz" ]; then
    read -p "Restore runtime config (~/.openclaw/)? [Y/n]: " RESTORE_RUNTIME
    RESTORE_RUNTIME=${RESTORE_RUNTIME:-Y}
    if [ "$RESTORE_RUNTIME" = "y" ] || [ "$RESTORE_RUNTIME" = "Y" ]; then
        echo "Restoring runtime config..."
        tar -xzf "$BACKUP_DIR/openclaw-runtime.tar.gz" -C "$HOME"
    fi
fi

# Optionally restore source (usually not needed if using git)
if [ -f "$BACKUP_DIR/openclaw-src.tar.gz" ]; then
    read -p "Restore source code? (usually not needed) [y/N]: " RESTORE_SRC
    if [ "$RESTORE_SRC" = "y" ] || [ "$RESTORE_SRC" = "Y" ]; then
        echo "Restoring source..."
        tar -xzf "$BACKUP_DIR/openclaw-src.tar.gz" -C "$(dirname "$SCRIPT_DIR")"
    fi
fi
